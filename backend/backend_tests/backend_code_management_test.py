...
def test_generate_new_access_code_6_months():
    ...
    assert abs(expected_expiration_time - code.expirationTime) < timedelta(seconds=2)

    assert code.activationTime == None
    assert code.isUsed == False
    assert code.usedByUser == None
...
def test_activating_unused_code():
    ...
    assert abs(activated_code.creationTime - expeted_code.creationTime) < timedelta(seconds=2)

    assert abs(activated_code.expirationTime - expeted_code.expirationTime) < timedelta(seconds=2)

    assert abs(activated_code.activationTime - expeted_code.activationTime) < timedelta(seconds=2)
