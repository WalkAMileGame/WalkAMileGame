...
def test_get_current_active_user_success(
        mock_jwt_decode, mock_datetime_class, mock_db):
    ...
    user = security.get_current_active_user(
        response=mock_response, token=test_token)
...
def test_get_current_active_user_token_refresh(
        mock_create_token, mock_jwt_decode, mock_datetime_class, mock_db):
    ...
    # Token issued 20 mins ago (past 15 min threshold)
    iat_time = mock_now - timedelta(minutes=20)
    mock_payload = {
        "sub": "test@example.com",
        "role": "user",
        "iat": iat_time.timestamp()
...
def test_get_current_active_user_token_expired(mock_jwt_decode):
    ...
    with pytest.raises(HTTPException) as exc_info:
        security.get_current_active_user(
            response=mock_response, token="expired.token")
...
def test_get_current_active_user_invalid_token(mock_jwt_decode):
    ...
    with pytest.raises(HTTPException) as exc_info:
        security.get_current_active_user(
            response=mock_response, token="invalid.token")
...
def test_get_current_active_user_invalid_payload_sub(mock_jwt_decode):
    ...
    with pytest.raises(HTTPException) as exc_info:
        security.get_current_active_user(
            response=mock_response, token="no.sub.token")
...
def test_get_current_active_user_invalid_payload_iat(mock_jwt_decode):
    ...
    with pytest.raises(HTTPException) as exc_info:
        security.get_current_active_user(
            response=mock_response, token="no.iat.token")
...
def test_get_current_active_user_not_found(mock_jwt_decode, mock_db):
    ...
    with pytest.raises(HTTPException) as exc_info:
        security.get_current_active_user(
            response=mock_response, token="ghost.user.token")
